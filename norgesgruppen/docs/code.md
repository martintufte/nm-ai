...
```bash
uv run python -m norgesgruppen.experiment train --name classify_v8n_1280_fulltrain --model-size n --epochs 120 --imgsz 1280 --batch -1 --multi-class --lr0 0.001 --mosaic 0.5 --mixup 0.1 --optimizer AdamW --patience 30 --val-fraction 0.1
```
...
docker run --rm -v "$PWD/norgesgruppen/submissions:/app/submissions" -v "$PWD/output:/output" -v "$PWD/norgesgruppen/data/images:/data/images" \
  ng-submission-test /app/submissions/20260320_235025/submission/run.py --input /data/images --output /output/predictions.json
```
